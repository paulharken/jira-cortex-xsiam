...
class Config:
    ...
    def from_params(cls) -> "Config":
        # V2: Uses demisto.getParam() which may natively unwrap encrypted fields
        return cls(
            cortex_base_url=(demisto.getParam("cortex_base_url") or "").rstrip("/"),
            cortex_api_key=demisto.getParam("cortex_api_key") or "",
            cortex_api_key_id=str(demisto.getParam("cortex_api_key_id") or ""),
            cortex_console_url=(demisto.getParam("cortex_console_url") or "").rstrip("/"),
            cortex_case_domain=demisto.getParam("cortex_case_domain") or "security",
            jira_site_url=(demisto.getParam("jira_site_url") or "").rstrip("/"),
            jira_cloud_id=demisto.getParam("jira_cloud_id") or "",
            jira_email=demisto.getParam("jira_email") or "",
            jira_api_token=demisto.getParam("jira_api_token") or "",
            jira_project_key=demisto.getParam("jira_project_key") or "",
            jira_issue_type=demisto.getParam("jira_issue_type") or "Alert",
            jira_case_id_field=demisto.getParam("jira_case_id_field") or "",
            jira_issue_id_field=demisto.getParam("jira_issue_id_field") or "",
            jira_xdr_url_field=demisto.getParam("jira_xdr_url_field") or "",
            resolution_type_map=demisto.getParam("resolution_type_map") or '{"False Positive": "Resolved - False Positive", "Duplicate": "Resolved - Duplicate Case", "Known Issue": "Resolved - Known Issue", "Security Testing": "Resolved - Security Testing", "TP Malicious": "Resolved - TP Malicious", "TP Benign": "Resolved - TP Benign", "SPAM": "Resolved - SPAM or Marketing"}',
            default_resolution_type=demisto.getParam("default_resolution_type") or "Resolved - Other",
            max_sync_cases=int(demisto.getParam("max_sync_cases") or "0"),
            sync_issues=demisto.getParam("sync_issues") or False,
            sync_from_date=demisto.getParam("sync_from_date") or "",
        )
